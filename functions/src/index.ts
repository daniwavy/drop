...
export const trackActiveTodayUsers = functions.firestore.onDocumentWritten(
  { document: 'users/{uid}/countersByDay/{dayId}', region: 'us-central1' },
  async (event) => {
    try {
      ...
      await db.collection('partners').doc(inviterUid).collection('activeTodayUsers').doc(uid).set(
...
export const resetActiveTodayUsers = functions.scheduler.onSchedule(
  { schedule: '0 0 * * *', timeZone: 'Europe/Berlin', region: 'us-central1' },
  async () => {
    try {
