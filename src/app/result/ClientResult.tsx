...
export default function ClientResult() {
  ...
  const capped = Boolean(
    payload?.capped === true ||
    (typeof payload?.room === 'number' && payload.room <= 0) ||
    (typeof payload?.current === 'number' && payload.current >= 100)
  );
  ...
  const hasDouble = Boolean(
    ...
    payload?.x2 === true ||
    payload?.tix2x === true ||
    Number(payload?.mult ?? payload?.multiplier ?? 1) >= 2
  );
  ...
  // Progressbar 0 → Max
  const scoreMax = clampNum(payload?.scoreMax ?? 1000, 1000);
  ...
  const raffleStats = React.useMemo(() => {
    const baseMin = Math.max(1, clampNum(payload?.ticketsMin, 50));
    if (!payload) {
...
function clampNum(v: string | number | null | undefined, fallback: number) {
