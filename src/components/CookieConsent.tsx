    ...
    <div style={{position:'fixed',right:16,bottom:16,zIndex:99999,maxWidth:420}}>
      <div style={{background:'#111',color:'#fff',padding:16,borderRadius:12,boxShadow:'0 8px 30px rgba(0,0,0,0.4)'}}>
        <div style={{display:'flex',flexDirection:'column',alignItems:'center',gap:8,marginBottom:8}}>
          <img src="/cookie.svg" alt="cookie" style={{width:48,height:48}} />
          <div style={{fontWeight:700,fontSize:16}}>Cookies & Datenschutz</div>
        </div>
  <div style={{fontSize:13,opacity:0.9,marginBottom:12,textAlign:'center'}}>Wir verwenden notwendige Cookies und optional Analytics/Marketing, um die Seite zu verbessern. Du kannst auswählen.</div>
        <div style={{display:'flex',gap:8,justifyContent:'flex-end'}}>
          <button onClick={acceptNecessary} className="cursor-pointer" style={{background:'#fff',color:'#000',borderRadius:8,padding:'8px 12px',fontWeight:600}}>Nur notwendige</button>
          <button onClick={() => setOpen(true)} className="cursor-pointer" style={{background:'transparent',color:'#fff',borderRadius:8,padding:'8px 12px',border:'1px solid rgba(255,255,255,0.12)'}}>Einstellungen</button>
          <button onClick={acceptAll} className="cursor-pointer" style={{background:'#10B981',color:'#fff',borderRadius:8,padding:'8px 12px',fontWeight:700}}>Alle akzeptieren</button>
        </div>
      </div>
      ...
      {open && (
        <div style={{marginTop:8,background:'#fff',color:'#000',padding:12,borderRadius:12,boxShadow:'0 8px 30px rgba(0,0,0,0.2)'}}>
          ...
          <div style={{display:'flex',gap:8,justifyContent:'flex-end',marginTop:8}}>
            <button onClick={()=>{ setOpen(false); }} className="cursor-pointer" style={{background:'transparent',padding:'8px 12px',borderRadius:8}}>Abbrechen</button>
            <button onClick={()=>{ saveCustom(analytics, marketing); setOpen(false); }} className="cursor-pointer" style={{background:'#10B981',color:'#fff',padding:'8px 12px',borderRadius:8,fontWeight:700}}>Speichern</button>
          </div>
        </div>
      )}
    </div>
